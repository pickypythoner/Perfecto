...
#TODO: Fix to use post where it is supposed to
...
class Repository(APIBase):
    ...
    def uploadItem(self, repository, itemKey, data, admin=False, owner=None, group=None, overwrite=False, format=None, reponseFormat="json", **properties):
        ...
        uriStr = "/repositories/%s/%s?operation=upload" % (repository, itemKey)
        rslt = None
        params = {}
        if admin:
            params["admin"] = admin
        ...
        params["responseFormat"] = reponseFormat
        if properties:
            params.update({("property.%s" % k, v) for (k, v) in properties})
        uriStr = properParams(uriStr, urlencode(params))
        log.debug("params are '%s'" % uriStr)
        try:
            rslt = self.client.send_post(uriStr)
